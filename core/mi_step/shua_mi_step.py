...
mistep_cf_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'mi_step_config'))

logging.basicConfig(level=cf.com_config.LOG_LEVEL, format='%(asctime)s-%(levelname)s: %(message)s')

tool_content = '小米运动刷支付宝微信步数工具'
logging.info(cf.com_config.get_author_info(tool_content))
...
def login(user, password):
    ...
    :return: token, userid
    '''
    url_regis = "https://api-user.huami.com/registrations/+86" + user + "/tokens"
    login_headers = {
    ...
    response_regis = requests.post(url_regis, data=data_regis, headers=login_headers, allow_redirects=False)
    try:
        location = response_regis.headers["Location"]
        code = get_code(location)
    except Exception:
        return 0, 0

    url_login = "https://account.huami.com/v2/client/login"
    data_login = {
    ...
    response_login = requests.post(url_login, data=data_login, headers=login_headers).json()
    login_token = response_login["token_info"]["login_token"]
    userid = response_login["token_info"]["user_id"]

    return login_token, userid


def shua_step(user, passwd, step):
    '''
    刷步数主要功能
    :param user:
    :param passwd:
    ...
    result_msg = f"{user}:{response['message']},[{step}]" + '\n'
...
def run():
    ...
    mistep_config = configparser.ConfigParser()
    mistep_config.read(mistep_cf_path, encoding="utf-8")
    sections = mistep_config.sections()
    logging.info(sections)
    send_title = '刷步数成功'
    send_msg = ''
    for section in sections:
        if (TODAY < (mistep_config.get(section, 'start_date'))
                or (TODAY > mistep_config.get(section, 'end_date'))):
            continue
        ...
        logging.info(f'开始刷 {_name}')
        msg = shua_step(phone, pwd, step_num)
        logging.info(msg)
        send_msg += msg
...
if __name__ == '__main__':
    logging.info(f'****** {tool_content} 开始运行 ******')
    run()
    logging.info(f'****** {tool_content} 运行结束 ******')
